...
public class DutchNationalFlag {
    ...
    public static void dutchFlagPartition(int pivotIndex, List<Color> A) {
        ...
        while(middle <= large){
            if (A.get(middle).ordinal() < pivot.ordinal()){
                Collections.swap(A, middle++, small++); // here small is always the first middle
            }else if (A.get(middle).ordinal() > pivot.ordinal()){
                Collections.swap(A, middle, large--);
            }else {
                middle++;
            }
        }
